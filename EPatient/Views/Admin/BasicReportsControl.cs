...
namespace EPatient.Views.Admin
{
    public partial class BasicReportsControl : MetroFramework.Controls.MetroUserControl
    {
        ...
        public void FillDoctorsGrid()
        {
            ...
            dataTable.Columns.Add("کارمندان");
            dataTable.Columns.Add("تعداد درخواست");
            dataTable.Columns.Add("کل درآمد");

            foreach (var doctorReport in query)
            {
                DataRow row = dataTable.NewRow();
                row["کارمندان"] = doctorReport.UserNameSurname;
                row["تعداد درخواست"] = doctorReport.TotalReservations;
                row["کل درآمد"] = doctorReport.TotalPrice + " تومان";
        ...
        public void FillServicesGrid()
        {
            ...
            {
                DataRow row = dataTable.NewRow();
                row["خدمات"] = doctorReport.ServiceName;
                row["تعداد درخواست"] = doctorReport.TotalReservations;
                row["کل درآمد"] = doctorReport.TotalPrice + " تومان";
                dataTable.Rows.Add(row);
            }
        ...
        public void FillPatientsGrid()
        {
            ...
            {
                DataRow row = dataTable.NewRow();
                row["بیمار"] = doctorReport.PatientNameSurname;
                row["تعداد درخواست"] = doctorReport.TotalReservations;
                row["کل درآمد"] = doctorReport.TotalPrice + " تومان";
                dataTable.Rows.Add(row);
            }
        ...
        private void metroGridDoctor_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
