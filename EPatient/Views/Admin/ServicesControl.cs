...
namespace EPatient.Views.Admin
{
    public partial class ServicesControl : MetroFramework.Controls.MetroUserControl
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MetroFramework.MetroMessageBox.Show(this, ex.Message, "                                                                                                                   خطا\n                       ", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (MetroFramework.MetroMessageBox.Show(this, "\n\n                                                                                                                            آیا از حذف این رکورد مطمئن هستید؟", "                                                                                                                   خطا\n                       ",
        ...
        private void textName_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(textName.Text) || textName.Text.Length > 255)
            {
                e.Cancel = true;
                textName.Focus();
                errorProvider.SetError(textName, "لطفا نام سرویس را وارد کنید");
            }
            else
            {
                e.Cancel = false;
        ...
        private void textPrice_Validating(object sender, CancelEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                e.Cancel = true;
                textPrice.Focus();
                errorProvider.SetError(textPrice, "لطفا عدد اعشاری را وارد کنید");
                return;
            }
        ...
        private void textDuration_Validating(object sender, CancelEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                e.Cancel = true;
                textPrice.Focus();
                errorProvider.SetError(textDuration, "لطفا عدد طبیعی را وارد کنید");
                return;
            }
        ...
        private void cboPavilion_Validating(object sender, CancelEventArgs e)
        {
            if (cboPavilion.SelectedIndex < 0)
            {
                e.Cancel = true;
                cboPavilion.Focus();
                errorProvider.SetError(cboPavilion, "لطفا بخش را انتخاب کنید");
            }
            else
            {
                e.Cancel = false;
