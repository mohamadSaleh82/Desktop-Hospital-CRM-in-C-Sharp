...
namespace EPatient.Views.Staff
{
    public partial class FolderForm : MetroFramework.Forms.MetroForm
    {
        ...
        public FolderForm(int patientId)
        {
            ...
            textGender.Text = (_currentPatient.Gender) ? "مرد" : "زن";
        ...
        private DataTable FillTable()
        {
            ...
            dataTable.Columns.Add("آیدی");
            dataTable.Columns.Add("تاریخ");
            dataTable.Columns.Add("پذیرش");
            dataTable.Columns.Add("ترخیص");
            dataTable.Columns.Add("دکتر");
            dataTable.Columns.Add("خدمات");
            dataTable.Columns.Add("پایان");
            ...
            foreach (var reservation in reservations)
            {
                ...
                row["آیدی"] = reservation.Id;
                row["تاریخ"] = reservation.StartTime.ToShortDateString();
                row["پذیرش"] = reservation.StartTime.ToString("H:mm");
                row["ترخیص"] = reservation.EndTime.ToString("H:mm");
                row["دکتر"] = reservation.DoctorName + " " + reservation.DoctorSurname;
                row["خدمات"] = reservation.ServiceName;
                row["پایان"] = (reservation.Done) ? "Yes" : "No";
        ...
        private void gridViewFolder_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
